...
import { adminKeys, intentTreeQueryOptions, mappingsQueryOptions } from "../../lib/queries";
import { createMapping } from "../../lib/api";
import { relativeTime } from "../../lib/format";

export function MappingsScreen() {
  const [addOpen, setAddOpen] = useState(false);
  const { data: mappings = [], isError, isPending, refetch } = useQuery(mappingsQueryOptions());
  ...
  return (
    <div>
      <AdminPageHeader
        icon={ArrowRightLeft}
        title="查询术语映射"
        description="配置查询归一化的关键词映射规则，把用户表述定向到正确意图。"
        actions={<Button onClick={() => setAddOpen(true)}>新增映射</Button>}
      />
...
function AddMappingDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => createMapping({ term: term.trim(), intentNodeId }),
    onSuccess: async () => {
      ...
      <DialogContent>
        ...
        <form id="mapping-form" onSubmit={handleSubmit} className="grid gap-4">
          <div className="grid gap-1.5">
            ...
            {termError ? (
              <p id="map-term-error" className="text-caption text-error">
                {termError}
              </p>
            ) : null}
          </div>
          <div className="grid gap-1.5">
            ...
            {intentError ? (
              <p id="map-intent-error" className="text-caption text-error">
                {intentError}
              </p>
            ) : null}
          </div>
