...
调用兼容 OpenAI 规范的 /v1/rerank 端点（契约：POST /rerank，入参包含 model, query, documents；返回各分块相关性得分 relevance_score）。
...
class OpenAICompatReranker:
    ...
    async def rerank(self, query: str, candidates: tuple[Candidate, ...]) -> tuple[Candidate, ...]:
        """对传入的候选分块集合执行二次打分并回填 reranker_score。"""
        if not candidates:
            return ()
        ...
        # 防御性解析：第三方 /rerank 实现可能省略 index 或返回非数值分数；
        # 整段响应不可用时抛 ProviderUnavailableError，由上层回落到通道分数。
        ...
        for item in results:
            ...
            try:
                ...
                continue
        if not scores:
            raise ProviderUnavailableError("invalid_rerank_response")
        return tuple(
            Candidate(
                chunk_id=c.chunk_id,
