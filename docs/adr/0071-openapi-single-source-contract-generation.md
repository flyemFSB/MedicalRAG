...
`/api/agent` is exempt from generation. Its contract is Agent Protocol v2, a standardized protocol consumed through official adapters (`@assistant-ui/react-langchain`, `@langchain/react`, `@langchain/langgraph-sdk`); `packages/contracts` only pins the protocol version and the application event envelope rather than generating a second, parallel description of it.
...
- `packages/contracts/typescript` contains generated types under a `generated/` marker plus a small set of hand-written shared structures (Evidence object, intent slots, run status) that the OpenAPI schema cannot express cleanly.
- `packages/contracts/python` holds the Pydantic models for the application event envelope and cross-process schemas; it does not re-derive the `/api` types.
