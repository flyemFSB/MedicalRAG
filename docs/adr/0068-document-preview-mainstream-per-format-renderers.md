...
- `.xlsx` is **typed unsupported** in v1 after SheetJS CE was removed (stale npm line + known CVE; see ADR 0080);
...
- `apps/web` depends on `pdfjs-dist` and `docx-preview` only (dynamic import); SheetJS is not installed.
- `.pptx` and `.xlsx` sources receive the typed unsupported-source result at the preview boundary (they remain ingested via MinerU; only in-UI preview is unsupported in v1).
- Objects are served from the application ObjectStorage path with expiring same-origin access; no document text is baked into HTML previews.
